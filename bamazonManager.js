...
// promptManagerAction will present menu options to the manager and trigger 
	...
	inquirer.prompt([
		{
			...
			choices: ['View Products for Sale', 'View  Inventory that is low on stock', 'Add to Inventory', 'Add New Product'],
			filter: function (val) {
				if (val === 'View Products for Sale') {
					return 'sale';
				} else if (val === 'View  Inventory that is low on stock') {
					return 'lowInventory';
	...
	inquirer.prompt([
		{
			type: 'input',
			name: 'item_id',
			message: 'Please enter the Item ID for stock counter update.',
		...
		connection.query(queryStr, {item_id: item}, function(err, data) {
			...
			if (data.length === 0) {
				console.log('ERROR: ID # does not exist. Please select a valid Item ID.');
				addInventory();
